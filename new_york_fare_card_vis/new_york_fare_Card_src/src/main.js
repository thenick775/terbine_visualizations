...
var formatAsP = d3.format("%"),
    ...
    diam = 700,
    labelhang = 630, //next 3 are for bar chart external labels
    start = 2000,
    intst = 127;
...
function responsivefy(svg) {
    ...
    function resize() {
        ...
        svg.attr('width', w);
        svg.attr('height', parseInt(container.style('height')));
    }
}

function responsivefy2(svg) {
    const container = d3.select(svg.node().parentNode),
        width = parseInt(svg.style('width'), 10),
        height = parseInt(svg.style('height'), 10),
        aspect = width / height;
    ...
    function resize() {
        const w = parseInt(container.style('width'));
        svg.attr('width', w);
        svg.attr('height', parseInt(container.style('height')) * 1.5);
    }
}
...
function dsPieGen() {
    ...
    var data = d3.csv("http://localhost:8765/data/dfpie.csv", function(dataset) { //create within this function, as dataset is only accessible here after ajax call finishes
        ...
        var outerRadius = diam / 2,
            ...
            color = d3.scaleOrdinal(d3.schemeCategory20c) //version c,range of colors
        ;

        var vis = d3.select("#pieChart")
            .append("svg:svg") //create the SVG element inside the <body>
            .data([dataset]) //associate our data with the document
            .attr("width", "800px") //set the width and height
            .attr("height", "800px")
        ...
        vis.append("svg:text")
            ...
            .attr("style", "font-size: 27;")
            .attr("class", "title").lower()
        //.style("z-index", -6);
...
function dsBarChartConfig() {
    var margin = {
        ...
        colorBar = d3.scaleOrdinal(d3.schemeCategory20c),
        barPadding = 3;
...
d3.csv("http://localhost:8765/data/dfbar.csv", function(dataset) { //create within this function, as dataset is only accessible here after ajax call finishes
    ...
    function dsBarChart() {
        ...
        //tooltip
        var barcharttooltip = d3.select("body").append("div")
        ...
        function barmouseover(d, i) {
            ...
            console.log(d)
            console.log("i:" + i)
            k = Object.keys(dataset[0])[i + 1]
            barcharttooltip.html("Total: " + d + "<br/>Type: " + k)
                .style("left", (d3.event.pageX-100) + "px")
                .style("top", (d3.event.pageY * 0.8) + "px");
        }
        ...
        svg.append("text")
            ...
            .attr("y", 35)
            .attr("class", "title")
            .attr("text-anchor", "middle").attr("style", "font-size: 25;")
        ...
        for (x in dataset[0]) {
            if (x != "station") {
                ...
                svg.append("text")
                    ...
                    .text(x).style("text-anchor", "start").attr("style", "font-size: 15;")
                    .attr('transform', 'rotate(45,' + start + "," + labelhang + ')');
                start = start - intst
            }
        }
...
function updateBarChart(group, colorChosen) {
    ...
    plot.selectAll("text.xAxis")
        ...
        .duration(750)
        .text(function(d) {
            return formatAsInt(Math.round(d));
        })
        .attr("text-anchor", "middle")
        // Set x position as left edge of each bar+half bar width
        .attr("x", function(d, i) {
