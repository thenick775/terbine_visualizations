function run(filedf,startposf) {
    d3.csv(filedf).then(function(datastrain) { //d3 v6 uses promises so changed here
        ...
        d3.select("#selectButton").on("change", function(d) {
            ...
            run("http://localhost:8765/maindf_" + selectedOption + ".csv",startpos); //adjust based on selected strainmeter
        })
...
run("http://localhost:8765/maindf_B079.csv",startpos); //trigger initial run